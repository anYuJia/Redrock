...
const initInfo = function (json) {
    document.querySelector('#nickName').value = json.profile.nickname;
    document.querySelector('#Infomation').value = json.profile.signature;
    let sex = document.getElementsByName("sex")
    for (let i = 0; i < sex.length; i++) {
        if(sex[i].value == json.profile.gender){
            sex[i].checked = 'checked';
        }
    }
}

const setLi = function () {
    document.querySelector('#saveInfo').addEventListener('click',saveInfo,true);
    //document.querySelector('#cancerInfo').addEventListener('click',cancerInfo,true);
}
const saveInfo = function () {
    ...
    let year = document.querySelector("#birthday-year");
    let year_index = year.selectedIndex;
    let myYear = year.options[year_index].value;
    let month = document.querySelector("#birthday-month");
    let month_index = month.selectedIndex;
    let myMonth = month.options[month_index].value;
    let day = document.querySelector("#birthday-day");
    let day_index = day.selectedIndex;
    let myDay = day.options[day_index].value;
    let birthday = getUnixTime(myYear+'-'+myMonth+'-'+myDay)*1000;
    let nickName = document.querySelector('#nickName').value;
    let Infomation = document.querySelector('#Infomation').value;
    let url = "http://ananqiexiafan.icu:3000/user/update?gender="+gender+"&signature="+Infomation+"&nickname="+nickName+"&birthday="+birthday;
    console.log(url)
    fetch(url,{
        method:'get',
        //Authorization: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJJRCI6MjAyMTIxMjY3MiwiTmFtZSI6IuadqOm5j-WuhyIsIlFRIjoiMjI1OTIwMjA0MCIsImV4cCI6MTY3MzMxNTE3OCwiaWF0IjoxNjczMzExNTc4LCJpc3MiOiJqb3NodWEifQ.t3IXSZ0YJC3oe4KWECqiZbB0-VkrbJbWio5s7a9wVNE",
...
const s = function (json) {
    if(json.code==200){
        alert('修改成功');
    }else if(json.code==302){
        alert(json.code+'修改失败,昵称不符合规范');
    }
    else{
        alert('修改失败'+json.msg);
    }
}
const cancerInfo = function () {
    alert('ok');
}
...
const updateTx = function () {
    ...
    async function upload(file) {
        ...
        let url = `http://ananqiexiafan.icu:3000/avatar/upload`
        fetch(url,{
            method: 'post',
            Authorization: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJJRCI6MjAyMTIxMjY3MiwiTmFtZSI6IuadqOm5j-WuhyIsIlFRIjoiMjI1OTIwMjA0MCIsImV4cCI6MTY3MzMxNTE3OCwiaWF0IjoxNjczMzExNTc4LCJpc3MiOiJqb3NodWEifQ.t3IXSZ0YJC3oe4KWECqiZbB0-VkrbJbWio5s7a9wVNE",
