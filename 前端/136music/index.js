...
const setUserGD = function (json) {
    ...
    document.querySelector('#myLove').href = "/gedan/index.html?id="+json['playlist'][0]['id'];
    document.querySelector('#myLoveName').innerText = json['playlist'][0]['name'];
    document.querySelector('#myLoveImg').src = json['playlist'][0]['coverImgUrl'];
...
const getGD = function(playListId){
    let url = "http://ananqiexiafan.icu:3000/related/playlist?id="+playListId;
    fetch(url,{
        method:'get',
    })
    .then(res => res.json())
...
const getBanner = function (data) {
    ...
    for (let i = 0; i < data['banners'].length; i++) {
        bannerList[i].setAttribute('src', data['banners'][i]['imageUrl']);
    }
}
...
const setBangDan = function (data) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        songName = document.querySelectorAll('.rank-list-body-song');
        singerName = document.querySelectorAll('.rank-list-body-name');
        for (let j = 0; j < data['list'][i]['tracks'].length; j++) {
            //alert(data['list'][i]['tracks'][j])
            songName[i * 3 + j].innerHTML = data['list'][i]['tracks'][j]['first'];
            singerName[i * 3 + j].innerHTML = data['list'][i]['tracks'][j]['second'];
            //console.log(data['list'][i]['tracks'][j]);
        }
...
const st = function () {
    ...
    var bod = document.getElementById('bod').getElementsByTagName('li')
    for (var i = 0; i < bod.length; i++) {
        bod[i].onmouseover = function () {
            clearInterval(clockTime)
            for (var j = 0; j < bod.length; j++) {
                if (bod[j] == this) {
                    num = j
                    liColor()
                    slowMove(fu.scrollLeft, img.clientWidth * num)
    ...
    function liColor() {
        ...
        bod[num].style.backgroundColor = 'red'
    }
