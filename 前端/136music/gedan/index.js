...
const getTitle = function(){
    let url = "http://ananqiexiafan.icu:3000/playlist/detail?id="+id;
    fetch(url,{
        method : 'get',
    }).then(res => res.json())
    .then(json => setTitle(json))
}
const setTitle = function (json) {
    document.querySelector('#gedanName').innerText = json['playlist']['name'];
    document.querySelector("#gedanimg").src = json['playlist']['coverImgUrl'];
    document.querySelector("#TX").src = json['playlist']['creator']['avatarUrl'];
    document.querySelector('#Nickname').innerHTML = json['playlist']['creator']['nickname'];
    document.querySelector('#createTime').innerText = new Date(json['playlist']['createTime'])
}
...
let setSong = function (songs, json) {
    ...
    for (let i = 0; i < json['songs'].length; i++) {
        let song = {};
        song.id = json['songs'][i]['id'];
        song.songName = json['songs'][i]['name'];
        song.singer = json['songs'][i]['ar'][0]['name'];
        song.zhuanji = json['songs'][i]['al']['name'];
        song.time = json['songs'][i]['dt']
        songs.push(song)
    }
    //console.log(songs)
}
...
const setSongs = function (songs) {
    ...
    let j = 0;
    document.querySelector('#song-count').innerText = songs.length;
    for (let i = 0; i < songs.length; i++, j++) {
        ...
        let time_text = document.createTextNode(fix(parseInt((songs[i].time/1000)/60),2)+":"+fix(parseInt((songs[i].time/1000)%60),2));
        //console.log(songs[i].songName, songs[i].singer, songs[i].zhuanji)
    ...
    document.querySelector('.center').style.height = 88 + (j - 6) * 5 + 'vh';
}
const getGeComment = function () {
    let url = 'http://ananqiexiafan.icu:3000/comment/playlist?id=';
    //let id = '705123491';
    url = url + id;
    fetch(url, {
        method: 'get',
    }).then(res => res.json())
        .then(json =>setGeDanComment(json))
}
const setGeDanComment = function (json) {
    let Pl = document.querySelector('#center-comment-text');
    console.log(Pl)
    for (let i = 0; i < json['comments'].length; i++) {
    //console.log(json['comments'][i]['user']['avatarUrl'],json['comments'][i]['user']['nickname'],json['comments'][i]['content'])
        let div_row = document.createElement('div');
        let div_row_class = document.createAttribute('class');
        div_row_class.value = 'center-comment-row';
        div_row.setAttributeNode(div_row_class);
        let img_t = document.createElement('img');
        let img_t_src = document.createAttribute('src')
        img_t_src.value = json['comments'][i]['user']['avatarUrl'];
        img_t.setAttributeNode(img_t_src);
        div_row.appendChild(img_t);
        let span_name = document.createElement('span');
        let span_content = document.createElement('span');
        let span_name_class = document.createAttribute('class');
        let span_content_class = document.createAttribute('class');
        span_name_class.value = 'center-comment-row-name';
        span_content_class.value = 'center-comment-row-content';
        span_name.setAttributeNode(span_name_class);
        span_content.setAttributeNode(span_content_class);
        span_name.innerText = json['comments'][i]['user']['nickname'];
        span_content.innerText = json['comments'][i]['content'];
        div_row.appendChild(span_name);
        div_row.appendChild(span_content);
        Pl.appendChild(div_row);
    }
}
const addListener = function(){
    document.querySelector('#pl-button').addEventListener('click',sendComment)
}
const sendComment=function(){
    if(document.querySelector('#center-comment-body-input').value == ""){
        alert('输入不能为空噢！');
        return
    }
    let url = "http://ananqiexiafan.icu:3000/comment?t=1&type=2&id="+id+"&content="+document.querySelector('#center-comment-body-input').value;
    fetch(url,{
        method:"get",
        Authorization: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJJRCI6MjAyMTIxMjY3MiwiTmFtZSI6IuadqOm5j-WuhyIsIlFRIjoiMjI1OTIwMjA0MCIsImV4cCI6MTY3MzMxNTE3OCwiaWF0IjoxNjczMzExNTc4LCJpc3MiOiJqb3NodWEifQ.t3IXSZ0YJC3oe4KWECqiZbB0-VkrbJbWio5s7a9wVNE",
        Cookie: "Cookie_1=value; MUSIC_U=bae943925ad3aa0db65486abd1b6e152eb0e6d5c8bc29bc76f9b3c363869b8e81e8907c67206e1ed03fd35b11ab07e7e6054c1a724ad28aedbc4e40dc68540b27895fe1ce8c630f0d4dbf082a8813684; NMTID=00ObHqyrDKrS-JodUEonotNs7xX07YAAAGFrsuYEQ; __csrf=c692f0a5644f297b6d89f2ac5644dd64",
    })
    document.querySelector('#center-comment-body-input').value = "";
    alert('发布成功');
}
...
const setControl = function () {
    ...
    document.querySelector('#start').addEventListener('click',playAll,true)
